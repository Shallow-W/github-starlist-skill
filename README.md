...
- 列出所有 star list
- 读取指定 list 中的仓库（支持分页，可处理超过 50 个仓库的 list）
- Star 仓库并添加到指定 list
- 从 list 中移除仓库
- 取消 star 仓库
- 创建/删除/重命名 list
...
1. 克隆到本地
2. 将 `skills/github-starlist/SKILL.md` 的路径加入你的 Claude Code skill 配置
...
- "取消 star https://github.com/foo/bar"
- "把 skill list 重命名为 tools"
...
- `updateUserList` — 重命名 list

分页支持通过 `pageInfo { hasNextPage endCursor }` 实现，可自动翻页获取大型 list 的全部仓库。
